...
> 本仓库是 [go-cqhttp/for-java](https://github.com/cssxsh/bot-core) 框架的 **BoBoJiangs 定制维护版**，
> 服务于 [qqbot](https://github.com/BoBoJiangs/qqbot) 修仙群机器人项目。
> 原框架使用文档见下方（保留原 README）。

## 定制版与原版的差异

- **SnowLuma 协议端兼容**：`MarkdownMessage` 继承 `TextMessage`（`getText()` 返回 markdown 内容），
  使业务侧 `getMessageByType(TextMessage.class)` 无需区分协议端即可解析 markdown 卡片消息
  （SnowLuma 下游戏回复为 markdown 卡片，NapCat 下为文本段）
- 保留线上运行版全部定制：BotConfig 扩展配置、WsBotClient 连接健康检查与重连策略、
  消息缓存优化、按钮注入（MessageButtonInjector）等
- `lib` 使用方式：本工程 `mvn clean package` 构建出的
  `bot-core/target/bot-core-1.0.0.jar` 拷贝到 qqbot 项目 `lib/` 目录后重新打包业务项目

## 版本对应

| tag | 说明 |
|-----|------|
| v1.0.0-synced | 与 qqbot `lib/bot-core-1.0.0.jar`（2026-08-31）完全同步的源码状态 |

⚠️ 修改本工程后务必重新构建并同步 jar 到 qqbot/lib，两边同一次提交，避免版本漂移。
